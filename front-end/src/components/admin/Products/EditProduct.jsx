...
function EditProduct() {
    ...
    useEffect(()=>{
        ...
        axiosClient.get(`/api/edit-product/${id}`).then((res)=>{
            if(res.data.status === 200){
                setProductInput(res.data.product);
                setCheckedData(res.data.product);
                setLoading(false)            
    ...
    const handleProductUpdate = (e)=>{
        ...
        formData.append('featured',checkedData.featured ? '1' : '0');
        formData.append('popular',checkedData.popular ? '1' : '0');
        formData.append('status',checkedData.status ? '1' : '0');
        
        axiosClient.post(`/api/update-product/${id}`,formData).then((res)=>{
                ...
                <div className="tab-pane fade card-body border p-4" id="details-tab-pane" role="tabpanel" aria-labelledby="seo-tab" tabIndex="0">
                    <div className='row'>
                        ...
                        <div className="mb-3 col-md-8">
                            ...
                            <img src={`http://127.0.0.1:8000/${productInput.image}`} alt='img' width='40px'/>
                            <div className='text-danger'>{error.image}</div>
                        </div>
                        <div className="form-check col-md-4">
                            <input name="featured"  className="form-check-input" onChange={handleCheckbox} defaultChecked={checkedData.featured === 1 ? true : false} type="checkbox" id="flexCheckDefault"/>
                                <label className="form-check-label" htmlFor="flexCheckDefault">
                                        featured
                                </label>
                        </div>
                        <div className="form-check col-md-4">
                            <input name="popular" onChange={handleCheckbox} defaultChecked={checkedData.popular === 1 ? true : false} className="form-check-input" type="checkbox"  id="flexCheckChecked"  />
                            <label className="form-check-label" htmlFor="flexCheckChecked">popular</label>
                        </div>
                        <div className="form-check col-md-4">
                            <input name="status" className="form-check-input" type="checkbox" onChange={handleCheckbox} defaultChecked={checkedData.status === 1 ? true : false} id="flexCheckChecked3"  />
